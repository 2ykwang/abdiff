...
이 도구는 규칙을 넣기 전과 넣은 후에 같은 작업을 각각 시키고, 두 결과를 나란히 두고 비교해서 보여줍니다.
...
- 실험은 임시 복사본에서 권한 확인 없이 실행됩니다. 이 복사본은 샌드박스가 아니라서 Bash 명령은 복사본 밖의 파일에도 영향을 줄 수 있습니다. 신뢰할 수 없는 프로젝트라면 `permission` 필드로 도구를 제한합니다(`skills/abdiff/references/protocol.md` 참고).
