...
2. Use the statement's own words for `statement`, dropping only the condition name and the direction. "Only the variant appends [X]" becomes "appends [X]"; "A and B" stays one finding "A and B". Don't paraphrase: the person comparing runs lines up findings across runs by this text. The one exception to "don't split": a statement that gives a value per condition ("baseline: 3 attempts, variant: 2") becomes one finding per value ("max attempts = 3", "max attempts = 2"). Never try to work out which condition this run is; that comparison is the person's job.
