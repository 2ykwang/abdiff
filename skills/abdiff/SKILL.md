...
> 8. LLM reading: off. (If on, an isolated Claude call checks each statement in "Expected" against each run and cites the record. It doesn't compare conditions or make the verdict. Adds <2 × repeats × cases> calls, each capped at $0.50 and 180 s. Model: `<different from item 2 when possible>`)
>
...
- If item 8 is on, every "Expected" must name the behavior to look for in one run, plus the direction. "Appends [X] to the answer; expected in no run" works. "No effect" alone doesn't: a reader of one run can't check it. Re-check this if the user turns item 8 on in their answer.
...
Save the answers to `.abdiff/<name>/experiment.json`. The format is in the "experiment.json" section of [references/protocol.md](references/protocol.md). Use the `permission` field if allowlist mode is needed, and the `reading` field if item 8 is on.
...
### 3. LLM reading (only if item 8 is on)

```bash
python3 "${CLAUDE_PLUGIN_ROOT}/skills/abdiff/scripts/reading.py" .abdiff/<name>
```

For each run, an isolated `claude -p` (no tools, no project or user instruction files, not told which condition the run is, the hypothesis, `variant.patch`, or `instructions.jsonl`) reports which statements in `expect` the run shows and cites where. It writes `reading.json` next to each run. What it sees and why is in [references/reading.md](references/reading.md). Use this script only; don't do the reading in this session or through the Agent tool, because both carry the project's CLAUDE.md, which is the variable under test.

It runs one call per run, sequentially, so run it in the background like the runner and watch stderr. Exit 1 after the calls means some readings failed: build the report anyway (they show as failed), tell the user how many, and don't rerun on your own, because a rerun repeats every call. Exit 130 means it was interrupted. Readings of runs the user excludes (timeout, error) are excluded with them.
