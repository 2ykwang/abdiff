...
#
# The checker (check.py) is run on every record and checked for a well-formed check.json.
# Cost: 4 claude-haiku-4-5 runs plus 4 check calls (a few cents). Requires: logged-in claude CLI, git, python3
...
CHECK_RC=0
python3 "$PLUGIN_DIR/skills/abdiff/scripts/check.py" .abdiff/e2e || CHECK_RC=$?
...
check "check.py exit code 0" test "$CHECK_RC" -eq 0
check "check.json in every run has findings with observed/evidence" python3 - <<'EOF'
import json, sys
ok = True
for run in ("TC-01/baseline", "TC-01/variant", "TC-02/baseline", "TC-02/variant"):
    j = json.load(open(f".abdiff/e2e/runs/{run}/1/check.json"))
...
sys.exit(0 if ok else 1)
EOF
check "checker ran isolated: no tools, no setting sources, strict MCP, auto memory off (checks.json command)" bash -c 'grep -q -- "CLAUDE_CODE_DISABLE_AUTO_MEMORY=1 claude -p .*--tools '"'"''"'"' --setting-sources '"'"''"'"' --strict-mcp-config" .abdiff/e2e/checks.json'
check "report shows the LLM check per run, collapsed" bash -c '[ "$(grep -o "<details class=\"instr\"><summary>LLM check" .abdiff/e2e/report.html | wc -l | tr -d " ")" = 4 ] && ! grep -q "<details class=\"instr\" open><summary>LLM check" .abdiff/e2e/report.html'
